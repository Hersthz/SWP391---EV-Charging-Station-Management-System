...
export default function ReservationDeposit() {
    ...
    // read params from router state or query string
    const init: InitParams = useMemo(() => {
        const state = (location.state || {}) as Partial<InitParams>;
        const sp = new URLSearchParams(location.search);
        return {
            reservationId: Number(state.reservationId ?? sp.get("reservationId")),
            amount: Number(state.amount ?? sp.get("amount")),
            stationName: String(state.stationName ?? sp.get("stationName") ?? ""),
            portLabel: String(state.portLabel ?? sp.get("portLabel") ?? ""),
            connectorLabel: String(state.connectorLabel ?? sp.get("connectorLabel") ?? ""),
            startTime: String(state.startTime ?? sp.get("startTime") ?? ""),
            endTime: String(state.endTime ?? sp.get("endTime") ?? ""),
            description: String(state.description ?? sp.get("description") ?? "Reservation deposit"),
        };
    }, [location]);
    ...
    const onPay = async () => {
        ...
        setErr(null);
        try {
            const returnUrl = `${window.location.origin}/api/payment/payment-return`;
            const body = {
                amount: init.amount,
                returnUrl,
                locale: "en",
