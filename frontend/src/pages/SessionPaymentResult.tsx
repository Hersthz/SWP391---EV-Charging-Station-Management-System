...
export default function SessionPaymentResult() {
  ...
  const result: Result = useMemo(() => {
    ...
    const message =
      ...
      sp.get("message") ??
      (status === "SUCCESS"
        ? "Charging session payment completed successfully."
        : sp.toString()
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center gap-3">
              {ok ? (
                ...
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center shadow bg-gradient-to-br from-amber-500 to-sky-500">
              ...
              <div>
                <CardTitle className={`text-2xl ${ok ? "text-emerald-700" : pending ? "text-amber-700" : "text-rose-700"}`}>
                  {ok ? "Payment Successful" : pending ? "Payment Pending" : "Payment Failed"}
                </CardTitle>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-4 rounded-xl border bg-white/60">
                ...
                <div className={`font-bold ${ok ? "text-emerald-600" : pending ? "text-amber-600" : "text-rose-600"}`}>
            ...
            {ok ? (
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm">
                Payment is confirmed. Your receipt has been recorded. You can return to Dashboard or view booking history.
              </div>
            ) : pending ? (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm">
                Your payment is processing. If the result doesn’t update automatically, please check again later or contact support.
              </div>
            ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <Button
                ...
                onClick={() => nav("/session/payment", { replace: true, state: savedInit })}
              >
